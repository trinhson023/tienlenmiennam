...
public sealed class LobbySeeder(LobbyDbContext db)
{
    ...
    [
        ...
        (Guid.Parse("22222222-2222-4222-8222-222222222222"), GameType.Sam, "sam-loc", "Sâm Lốc", "🃏", 2, 4, true),
    ...
    public async Task SeedAsync(CancellationToken ct = default)
    {
        ...
        {
            var existing = await db.Games.SingleOrDefaultAsync(x => x.Slug == item.Slug, ct);
            if (existing is null)
            {
                db.Games.Add(new GameDefinition(
                    item.Id,
                    item.Type,
                    item.Slug,
                    item.Name,
