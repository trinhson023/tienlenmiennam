...
public static class LobbyEndpoints
{
    public static IEndpointRouteBuilder MapLobbyEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        group.MapPost("/rooms/{roomId:guid}/leave", async (Guid roomId, ClaimsPrincipal principal, LobbyApplicationService service, IHubContext<LobbyHub> hub, CancellationToken ct) => { var userId = GetUserId(principal); if (userId is null) return Results.Unauthorized(); var result = await service.LeaveRoomAsync(roomId, userId.Value, ct); if (!result.IsSuccess) return Map(result); if (result.Value!.RoomRemoved) { await hub.Clients.Group("lobby").SendAsync("RoomRemoved", roomId, ct); await hub.Clients.Group($"room:{roomId}").SendAsync("RoomRemoved", roomId, ct); } else { await hub.Clients.Group("lobby").SendAsync("RoomChanged", roomId, ct); await hub.Clients.Group($"room:{roomId}").SendAsync("RoomUpdated", result.Value.Room, ct); } return Results.Ok(result.Value); });
        return endpoints;
    }
    ...
    private static IResult Map<T>(ServiceResult<T> result)
    {
        ...
        {
            "room_not_found" or "game_not_found" or "bot_not_found" or "rematch_match_not_found" or "match_not_found" => StatusCodes.Status404NotFound,
            "host_only" or "game_not_enabled" or "not_in_room" => StatusCodes.Status403Forbidden,
            "already_in_room" or "room_full" or "room_not_open" or "join_conflict" or "bot_conflict" or "not_enough_players" or "match_in_progress" or "rematch_not_available" or "rematch_not_ready" or "rematch_roster_changed" or "rematch_room_mismatch" or "match_room_mismatch" or "room_state_invalid" => StatusCodes.Status409Conflict,
