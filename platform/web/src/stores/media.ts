...
export const useMediaStore=defineStore('media',{state:()=>({hub:null as HubConnection|null,state:null as MediaRoomState|null,currentRoomId:null as string|null,error:''}),actions:{
  ...
  async search(q:string,shorts=false){if(!this.currentRoomId)throw new Error('Chưa vào phòng.');const{data}=await api.get<MediaTrack[]>(shorts?'/api/media/shorts':'/api/media/search',{params:{roomId:this.currentRoomId,q}});return data},
