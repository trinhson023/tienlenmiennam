...
namespace SamLocService.Api.Integration;

public sealed class MatchOutboxPublisherWorker(
    IDbContextFactory<SamLocDbContext> dbFactory,
    IBus bus,
    ILogger<MatchOutboxPublisherWorker> logger) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                logger.LogError(ex, "Sâm Lốc outbox publish loop failed.");
            }
    ...
    private async Task PublishBatchAsync(CancellationToken ct)
    {
        ...
        foreach (var record in records)
        {
            ...
            catch (Exception ex) when (!ct.IsCancellationRequested)
            {
                ...
                logger.LogWarning(
                    ex,
                    "Failed to publish Sâm MatchCompleted for match {MatchId}; attempt {Attempt}.",
                    record.MatchId,
                    record.Attempts);
            }
        }
