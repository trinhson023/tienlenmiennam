...
public sealed class TienLenMatchApplicationService(IMatchStore store, MatchRuntimeSettings settings)
{
    ...
    public async Task<CreateMatchResult> CreateMatchAsync(CreateMatchRequest request, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (existing is not null && existing.Match.Status == MatchStatus.InProgress) return CreateMatchResult.Success(existing.Match.Id.Value);
            ...
            var setups = orderedPlayers.Select((player, index) => new PlayerSetup(new PlayerId(player.UserId), new SeatNumber(player.SeatNumber), hands[index])).ToArray();
    ...
    public async Task<CreateMatchResult> CreateRematchAsync(Guid previousMatchId, CreateMatchRequest request, CancellationToken ct)
    {
        ...
        if (previous is null) return CreateMatchResult.Failure("rematch_match_not_found", "Không tìm thấy ván trước.");
        if (request.Players is null) return CreateMatchResult.Failure("invalid_players", "Danh sách người chơi không hợp lệ.");
        ...
        try
        {
            ...
            if (oldPlayers.Length != newPlayers.Length || oldPlayers.Zip(newPlayers).Any(pair => pair.First.UserId != pair.Second.UserId || pair.First.SeatNumber != pair.Second.SeatNumber || pair.First.IsBot != pair.Second.IsBot))
    ...
    public async Task<bool> ProcessAutomationAsync(Guid matchId, DateTimeOffset now, CancellationToken ct)
    {
        ...
        try {
            ...
            } else if (identity.IsBot && legal is not null) action = runtime.Match.PlayCards(player.Id, legal);
            else action = runtime.Match.Pass(player.Id);
    ...
    private async Task CommitActionAsync(MatchRuntime runtime, DateTimeOffset now, CancellationToken ct)
    {
        ...
        if (runtime.Match.Status == MatchStatus.Completed) {
            runtime.CompletedAtUtc ??= now; runtime.TurnDeadlineUtc = null; runtime.BotActionDueUtc = null;
        } else ScheduleTurn(runtime, now);
        await store.SaveAsync(runtime, ct);
    }

    private void ScheduleTurn(MatchRuntime runtime, DateTimeOffset now)
    {
        runtime.TurnDeadlineUtc = now.AddSeconds(settings.TurnSeconds); runtime.BotActionDueUtc = null;
    ...
    private static MatchStateView Project(MatchRuntime runtime, Guid viewerUserId)
    {
        ...
        var players = match.Players.OrderBy(x => x.Seat.Value).Select(player => {
            ...
            return new MatchPlayerView(player.Id.Value, player.Seat.Value, identity.Username, identity.DisplayName, player.Hand.Count, player.HasFinished, player.FinishPosition, player.Id.Value == viewerUserId, identity.IsBot);
        }).ToArray();
    ...
    {
        for (var i = cards.Length - 1; i > 0; i--) { var j = RandomNumberGenerator.GetInt32(i + 1); (cards[i], cards[j]) = (cards[j], cards[i]); }
    }
}
