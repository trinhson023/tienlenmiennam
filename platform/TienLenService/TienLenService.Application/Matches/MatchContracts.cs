...
public sealed record MatchPlayerView(Guid UserId, int SeatNumber, string Username, string DisplayName, int CardCount, bool HasFinished, int? FinishPosition, bool IsSelf, bool IsBot);
public sealed record MatchStateView(Guid MatchId, Guid RoomId, long Version, string Status, Guid? CurrentPlayerUserId, bool CurrentPlayerIsBot, DateTimeOffset? TurnDeadlineUtc, bool IsOpeningPlay, string? CenterType, IReadOnlyList<string> Center, IReadOnlyList<string> LastPlayedCards, IReadOnlyList<string> Hand, IReadOnlyList<MatchPlayerView> Players, IReadOnlyList<Guid> WinnerOrder);
